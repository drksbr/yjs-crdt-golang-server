...
export function AudioNotes({
    ...
    const audioNotesArray = useArray(subdocumentId && !useRootState ? `subdocuments.${subdocumentId}.audioNotes` : 'audioNotes')
        ...
        fetch(`/api/audio-notes?${queryParams.toString()}`, {
            ...
            .then((serverNotes: AudioNote[]) => {
                ...
                const existingIds = new Set(
                    Array.from(audioNotesArray).map((item: any) => item?.id).filter(Boolean),
                )
                const missing = serverNotes.filter((note) => note?.id && !existingIds.has(note.id))
                if (missing.length > 0) {
