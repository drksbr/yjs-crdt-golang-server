...
- [ ] Definir `snapshot + update log` como fonte de hidratação, replay, recuperação e troca de owner
- [ ] Persistir snapshot base e update log append-only por epoch, com replay determinístico e checkpoint/compaction planejados
- [ ] Definir protocolo inter-node próprio, separado do `y-protocols`, para resolve/open/forward/broadcast/hydrate/handoff/recovery
...
- [ ] Definir contratos mínimos de schema para snapshot base, update log e metadados de epoch/fencing
- [ ] Definir regras de recuperação após queda do owner e de promoção segura de novo owner
- [ ] Planejar smoke tests de multi-nó, handoff e restart com replay determinístico
